...
export function AdminEmptyState({ title, description, icon, action }: { title: string; description: string; icon?: ReactNode; action?: ReactNode }) {
  return <div className="admin-empty-state">
    {icon && <div className="admin-empty-state-icon" aria-hidden="true">{icon}</div>}
    <h2>{title}</h2>
    <p>{description}</p>
