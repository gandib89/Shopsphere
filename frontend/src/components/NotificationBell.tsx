...
interface Notification { id: string; type: 'new_product' | 'low_stock' | 'order_update' | 'discount'; title: string; message: string; read: boolean; productId?: string; productName?: string; productImage?: string; createdAt: string }
...
export default function NotificationBell() {
  ...
  const markRead = async (notification: Notification) => { if (notification.read) return; try { await request(`/api/v1/notifications/read/${notification.id}`, 'PUT'); setNotifications(previous => previous.map(item => item.id === notification.id ? { ...item, read: true } : item)); setUnreadCount(count => Math.max(0, count - 1)); } catch { setError('Could not update that notification.'); } };
  ...
  const dismiss = async (event: MouseEvent, notification: Notification) => { event.stopPropagation(); try { await request(`/api/v1/notifications/${notification.id}`, 'DELETE'); setNotifications(previous => previous.filter(item => item.id !== notification.id)); if (!notification.read) setUnreadCount(count => Math.max(0, count - 1)); } catch { setError('Could not dismiss that notification.'); } };
  ...
  return <div className="relative" ref={rootRef}>
    ...
    {open && <div ref={panelRef} id="notification-panel" role="dialog" aria-label="Notifications" className="absolute right-0 top-[calc(100%+8px)] z-[1000] w-[360px] max-w-[calc(100vw-16px)] overflow-hidden rounded-[var(--radius-surface)] border border-hairline bg-paper-raised shadow-float animate-panel-in">
      ...
      <div className="max-h-[420px] overflow-y-auto">{notifications.length === 0 ? <div className="px-4 py-10 text-center text-sm text-ink-muted"><Bell className="mx-auto mb-2 h-10 w-10 opacity-40" aria-hidden="true" />No notifications yet</div> : notifications.map(notification => <div key={notification.id} className={`flex items-stretch border-b border-hairline ${notification.read ? 'bg-paper-raised' : 'bg-brass-light/30'}`}>
