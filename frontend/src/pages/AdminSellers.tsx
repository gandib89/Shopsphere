  ...
  return <main>
    ...
    <section className="admin-panel" aria-label="Seller directory">
      <div className="admin-toolbar">
        ...
        <div className="admin-filters"><label>Status<select aria-label="Filter sellers by status" value={status} onChange={event => update({ status: event.target.value })}><option value="all">All sellers</option><option value="verified">Verified</option><option value="unverified">Not verified</option></select></label>{Boolean(data?.items.length) && (q || status !== 'all') && <button className="admin-button" onClick={() => { setDraft(''); setParams({}); }}>Clear filters</button>}</div>
      </div>
      {loading ? <p className="admin-empty" role="status">Loading sellers…</p> : error ? <p className="admin-empty">The seller directory is unavailable.</p> : !data?.items.length ? <AdminEmptyState icon={<Store />} title={q || status !== 'all' ? 'No matching sellers' : 'No seller accounts yet'} description={q || status !== 'all' ? 'Try another search or verification filter.' : 'Seller accounts will appear here once they register.'} action={q || status !== 'all' ? <button className="admin-button" onClick={() => setParams({})}>Clear filters</button> : <Link className="admin-button admin-button--primary" to="/admin/users">Add seller account</Link>} /> : <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th scope="col">Shop / owner</th><th scope="col">Contact</th><th scope="col">Status</th><th scope="col">Joined</th><th scope="col">Actions</th></tr></thead><tbody>{data.items.map(seller => <tr key={seller.id}><td><Link className="admin-text-link" to={'/admin/sellers/' + seller.id}>{sellerName(seller)}</Link><small>{seller.firstName} {seller.lastName}</small></td><td>{seller.email}<small>{seller.phone || 'No phone provided'}</small></td><td><span className={'admin-status admin-status--' + (seller.isVerified ? 'success' : 'attention')}>{sellerStatus(seller)}</span></td><td className="admin-numeric">{adminDate(seller.createdAt)}</td><td><Link className="admin-button" to={'/admin/sellers/' + seller.id} aria-label={'View seller ' + sellerName(seller)}>View seller</Link></td></tr>)}</tbody></table></div>}
