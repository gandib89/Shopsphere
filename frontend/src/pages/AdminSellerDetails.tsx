...
import { ArrowLeft, ClipboardList, Package, Plus, RefreshCw, Search } from 'lucide-react';
      ...
      {view === 'products' ? <section className="admin-panel" aria-label="Product catalogue">
        ...
        <div className="admin-toolbar">
          ...
          <div className="admin-filters">
            ...
            {filteredProducts.length > 0 && (search || category !== 'all' || stock !== 'all' || sort !== 'name' || direction !== 'asc') && <button className="admin-button" onClick={() => { setSearch(''); setCategory('all'); setStock('all'); setSort('name'); setDirection('asc'); }}>Clear filters</button>}
          </div>
        </div>
        {loading ? <p className="admin-empty" role="status">Loading products…</p> : error ? <p className="admin-empty">Refresh to load the catalogue.</p> : !filteredProducts.length ? <AdminEmptyState icon={<Package />} title={products.length ? 'No matching products' : 'No products from this seller yet'} description={products.length ? 'Try another search, category, or stock filter.' : 'Add the first listing on behalf of this seller.'} action={products.length ? <button className="admin-button" onClick={() => { setSearch(''); setCategory('all'); setStock('all'); setSort('name'); setDirection('asc'); }}>Clear filters</button> : <Link className="admin-button admin-button--primary" to={`/admin/sellers/${encodeURIComponent(sellerId)}/products/new`}>Add first product</Link>} /> :
        ...
        {error && <p className="admin-notice" role="alert">{error}</p>}
        <div className="admin-toolbar"><label className="admin-search"><Search size={15} aria-hidden="true" /><input type="search" aria-label="Search orders" placeholder="Search order, customer, email…" value={search} onChange={event => setSearch(event.target.value)} /></label><div className="admin-filters"><label>Sort by<select aria-label="Sort orders" value={sort} onChange={event => setSort(event.target.value)}><option value="newest">Date (newest first)</option><option value="name">Customer name</option><option value="product">Product name</option><option value="delivery">Delivery date</option><option value="total">Highest total</option></select></label>{filteredOrders.length > 0 && (search || status !== 'all') && <button className="admin-button" onClick={() => { setSearch(''); setStatus('all'); }}>Clear filters</button>}</div></div>
        {loading ? <p className="admin-empty" role="status">Loading orders…</p> : error ? <p className="admin-empty">Refresh to load the order list.</p> : !filteredOrders.length ? <AdminEmptyState icon={<ClipboardList />} title={orders.length ? 'No matching orders' : 'No orders for this seller yet'} description={orders.length ? 'Try another search or status filter.' : 'Orders will appear here after customers buy this seller’s products.'} action={orders.length ? <button className="admin-button" onClick={() => { setSearch(''); setStatus('all'); }}>Clear filters</button> : <Link className="admin-button admin-button--primary" to={`/admin/sellers/${encodeURIComponent(sellerId)}/products/new`}>Add a product</Link>} /> :
          <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th scope="col">Order / customer</th><th scope="col">Date</th><th scope="col">Status</th><th scope="col">Total</th><th scope="col">Actions</th></tr></thead><tbody>{filteredOrders.slice((current - 1) * 20, current * 20).map(order => <Fragment key={order._id}>
            ...
            {expanded === order._id && <tr id={'preview-' + order._id}><td colSpan={5}><div className="admin-order-preview"><h3>{order.product?.name || 'Order details'}</h3><dl><div><dt>Customer</dt><dd>{order.firstName} {order.lastName}<br />{order.email}</dd></div><div><dt>Expected delivery</dt><dd>{adminDate(order.deliveryDate)}</dd></div><div><dt>Quantity / configuration</dt><dd>{order.quantity} · {order.color || order.variants?.color || 'Standard'} {order.variants?.storage || ''}</dd></div></dl>
              ...
              <div className="admin-order-preview-actions"><Link className="admin-button admin-button--primary" to={'/admin/orders/' + order._id}>View full order</Link>
                ...
                {order.status === 'Return Approved' && <button className="admin-button" disabled={!!busy} onClick={() => void action(order, 'refund')}>Complete sandbox refund · {adminMoney(order.totalPrice)}</button>}
