...
export default function SellerRevenueDashboard() {
  ...
  return <main>
    ...
    {!error && (loading ? <section className="admin-panel"><p className="admin-empty" role="status">Loading revenue…</p></section> : months.length === 0 ? <section className="admin-panel"><AdminEmptyState icon={<BarChart3 />} title="No settled revenue yet" description="Sales and earnings will appear after your first order is delivered." action={<Link className="admin-button admin-button--primary" to="/seller-orders">View customer orders</Link>} /></section> : <><section className="admin-panel" aria-labelledby="seller-trend-title">
